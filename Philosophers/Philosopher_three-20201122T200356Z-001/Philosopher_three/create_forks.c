...
/*   Updated: 2020/11/14 17:47:03 by fvega-tr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_forks(t_philos *philo)
{
	pthread_t	tid;
...
int			ft_create_forks(t_struct *general)
{
	...
	{
		general->philo[i].pid = fork();
		if (general->philo[i].pid < 0)
			return (1);
		else if (general->philo[i].pid == 0)
		{
			ft_forks(&general->philo[i]);
			exit(0);
		}
		usleep(100);
		i++;
